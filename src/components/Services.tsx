  ...
  return (
    <section id="services" className="bg-[#0e0e0e] px-8 lg:px-14 py-24 border-t border-[#1a1a1a]">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
        <div>
          <p className="text-xs text-[#666] tracking-[0.2em] uppercase mb-5">What we do</p>
          <h2
        ...
        <p className="text-[#8a8a8a] text-base leading-relaxed max-w-xs">
        ...
        {services.map((s, i) => (
          ...
          >
            <div className="flex items-start gap-4 md:w-64 flex-shrink-0">
              <span className="text-xs text-[#555] font-mono mt-1 w-5 flex-shrink-0">{s.num}</span>
              <div className="mt-0.5 flex-shrink-0">{s.icon}</div>
              <h3 className="text-[#c0c0c0] font-medium text-base leading-snug">{s.title}</h3>
            </div>
            <p className="text-[#9a9a9a] text-base leading-relaxed max-w-xl flex-1">{s.description}</p>
            <div className="flex flex-wrap gap-2 md:w-52 flex-shrink-0 md:justify-end items-start">
              {s.tags.map((tag) => (
                <span key={tag} className="text-xs text-[#666] border border-[#252525] px-2.5 py-1 whitespace-nowrap">
                  {tag}
                </span>
              ))}
            </div>
